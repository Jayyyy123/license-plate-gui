...
def maximizeContrast(imgGrayscale):
    ...
    imgTopHat = cv2.morphologyEx(imgGrayscale, cv2.MORPH_TOPHAT,
                                 structuringElement)  # erode then dilate (opening) difference between opening and input
    imgBlackHat = cv2.morphologyEx(imgGrayscale, cv2.MORPH_BLACKHAT,
                                   structuringElement)  # dilate then erode (closing) difference between closing and input
