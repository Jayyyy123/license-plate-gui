...
def findPossibleCharsInScene(imgThresh):
    ...
    for i in range(0, len(contours)):
        if product2.showSteps:
            cv2.drawContours(imgContours, contours, i, product2.SCALAR_WHITE)
...
def extractPlate(imgOriginal, listOfMatchingChars):
    ...
    fltHypotenuse = char_detection.distanceBetweenChars(listOfMatchingChars[0],
...
def detectPlatesInScene(imgOriginalScene):
    ...
    if product2.showSteps:
        ...
        cv2.drawContours(imgContours, contours, -1, product2.SCALAR_WHITE)
    ...
    if product2.showSteps:
        ...
        for i in range(0, len(listOfPossiblePlates)):
            ...
            cv2.line(imgContours, tuple(p2fRectPoints[0]), tuple(p2fRectPoints[1]), product2.SCALAR_RED, 2)
            cv2.line(imgContours, tuple(p2fRectPoints[1]), tuple(p2fRectPoints[2]), product2.SCALAR_RED, 2)
            cv2.line(imgContours, tuple(p2fRectPoints[2]), tuple(p2fRectPoints[3]), product2.SCALAR_RED, 2)
            cv2.line(imgContours, tuple(p2fRectPoints[3]), tuple(p2fRectPoints[0]), product2.SCALAR_RED, 2)
            cv2.imshow("4a", imgContours)
